...
class UserMessagesService:
    ...
    def get_conversations(db: Session, user_id: str) -> List[dict]:
        ...
        try:
            ...
            sent_users = db.query(
                UserMessage.recevier_id.label("other_user_id")
            ).filter(
                UserMessage.sender_id == user_id
            )

            received_users = db.query(
                UserMessage.sender_id.label("other_user_id")
            ).filter(
                UserMessage.recevier_id == user_id
            )

            all_conversation_users = sent_users.union(
                received_users
            ).subquery()
            ...
            conversations = db.query(
                ...
                unread_subquery,
                User.id == unread_subquery.c.other_user_id
            ).all()
                ...
                if last_message:
                    conversation_list.append({
                        ...
                        "lastMessageTime": (
                            ...
                            if last_message.created_at else None
                        ),
